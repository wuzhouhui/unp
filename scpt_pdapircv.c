 ...
 * =============================================================================
 *
 *       Filename:  scpt_pdapircv.c
 *
 *    Description:  handling partial delivery API. figure 23-2,
 *		    page 491.
 *
 *        Version:  1.0
 *        Created:  04/06/2015 07:02:01 PM
 *       Revision:  none
 *       Compiler:  gcc
 *
 *         Author:  YOUR NAME (), 
...
static uint8_t	*sctp_padpi_readbuf = NULL;
		...
		struct sctp_sndrcvinfo *sri, int *msg_flags)
{
	...
	if (sctp_pdapi_readbuf == NULL) {
		sctp_pdapi_readbuf = (uint8_t *)Malloc(SCTP_SPAPI_INCR_SZ);
		sctp_pdapi_rdbuf_sz = SCTP_SPAPI_INCR_SZ;
	}
	at_in_buf = Sctp_recvmsg(sock_fd, sctp_padpi_readbuf, 
			sctp_pdapi_rdbuf_sz, from, from_len, sri, msg_flags);
	if (at_in_buf < 1) {
		*rdlen = at_in_buf;
		return NULL;
	}
	while ((*msg_flags & MSG_EOR) == 0) {
		...
		if (left < SCTP_PDAPI_NEED_MORE_THRESHOLD) {
			...
			if (sctp_pdapi_readbuf == NULL)
				err_quit("sctp_pdapi ran out of memeory");
			sctp_pdapi_rdbuf_sz += SCTP_PDAPI_INCR_SZ;
			left = sctp_pdapi_rdbuf_sz - at_in_buf;
		}
