 ...
 *       Filename:  tcpcli01.c
 *
 *    Description:  TCP echo client program that specifys a source path,
 *		    figure 27-6, page 566.
 *
 *        Version:  1.0
 *        Created:  04/11/2015 03:27:32 PM
 *       Revision:  none
 *       Compiler:  gcc
 *
 *         Author:  YOUR NAME (), 
...
int main(int argc, char **argv)
{
	...
	if (ptr)
		while (optind < argc - 1)
			len = inet_strcrt_add(argv[optind++]);
	...
	if (ptr) {
		len = inet_strcrt_add(argv[optind]); /* dest at end */
