 ...
 * =============================================================================
 *
 *       Filename:  tpi_connect.c
 *
 *    Description:  tpi_connect() function. for create connection to 
 *		    server. figure 31-10, page 684.
 *
 *        Version:  1.0
 *        Created:  04/24/2015 01:16:33 PM
 *       Revision:  none
 *       Compiler:  gcc
 *
 *         Author:  YOUR NAME (), 
...
void tpi_connect(int fd, const void *addr, size_t addrlen)
{
	...
	default:
		err_quit("tpi_connect: unexpected message type: %d", 
				rcvbuf.type);
	}
	...
	case T_DISCON_IND:
		if (ctlbuf.len < (int)sizeof(struct T_discon_ind))
			err_quit("tpi_connect2: bad length form T_DISCON_IND");
