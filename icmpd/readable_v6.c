 ...
 * =============================================================================
 *
 *       Filename:  readable_v6.c
 *
 *    Description:  handling ICMPv6 packet that received. figure 28-41,
 *		    page 620.
 *
 *        Version:  1.0
 *        Created:  04/17/2015 02:15:46 PM
 *       Revision:  none
 *       Compiler:  gcc
 *
 *         Author:  YOUR NAME (), 
...
int readable_v6(void)
{
	...
	printf("%d bytes ICPv6 from %s: ", n, Sock_ntop_host((SA *)&from,
			len));
		...
		if (hip6->ip6_nxt == IPPROTO_UDP) {
			...
			for (i = 0; i <= maxi; i++) {
				if (client[i].connfd >= 0 &&
					...
					/* convert type & code to reasonable
					 * errno value */
