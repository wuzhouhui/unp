 ...
 *       Filename:  pcap.c
 *
 *    Description:  open_pcap() function for open and initialize packet 
 *		    capturing device. figure 29-9, page 633.
 *
 *        Version:  1.0
 *        Created:  04/22/2015 02:39:34 PM
 *       Revision:  none
 *       Compiler:  gcc
 *
 *         Author:  YOUR NAME (), 
...
void open_pcap(void)
{
	...
	if (pcap_lookupnet(device, &localnet, &netmask, errbuf) < 0)
		err_quit("pcap_open_lookupnet: %s", errbuf);
	if (verbose)
