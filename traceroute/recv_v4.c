 ...
 *       Filename:  recv_v4.c
 *
 *    Description:  recv_v4(), read and handling ICMPv4 message.
 *		    figure 28-20, page 602.
 *
 *        Version:  1.0
 *        Created:  04/16/2015 01:45:18 PM
 *       Revision:  none
 *       Compiler:  gcc
 *
 *         Author:  YOUR NAME (), 
...
int recv_v4(int seq, struct timeval *tv)
{
	...
	for ( ; ; ) {
		...
		if ((icmplen = n - hlen1) < 8)
			continue; /* not enough to loop at ICMP header */
		...
		if (icmp->icmp_type == ICMP_TIMXCEED &&
			...
			if (icmplen < 8 + hlen2 + 4)
				continue; /* not enough data to look at UDP port */
			...
			udp = (struct udphdr *)(recvbuf + hlen1 + 8);
			hlen2 = hip->ip_hl << 2;
			if (icmplen < 8 + hlen2 + 4)
		...
		/* some other ICMP error, recvfrom() agagin. */
	}
