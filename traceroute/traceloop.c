 ...
 * =============================================================================
 *
 *       Filename:  traceloop.c
 *
 *    Description:  traceloop(), main handling loop. figure 28-19, page
 *		    600.
 *
 *        Version:  1.0
 *        Created:  04/15/2015 11:03:04 PM
 *       Revision:  none
 *       Compiler:  gcc
 *
 *         Author:  YOUR NAME (), 
...
void traceloop(void)
{
	...
	setuid(getuid());	/* don't need special premissions anymore. */
	...
	for (ttl = 1; ttl <= max_ttl && done == 0; ttl++) {
		...
		for (probe = 0; probe < nprobes; probe++) {
			...
			if ((code = (*pr->recv)(seq, &tvrecv)) == -3)
				...
				if (sock_cmp_addr(pr->sarecv, pr->salast,
						pr->salen) != 0) {
